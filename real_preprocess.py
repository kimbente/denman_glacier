...
def load_and_preprocess_ice_velocity_data(file_path):
    ...
    corners_byrd_regions = pd.read_csv("data/byrd/corner_coordinates_byrd_for_velocity.csv")
...
# function
def combine_thickness_and_velocity(thickness_points, velocity_grid, subsample_rate = 20):
    """
    ...
    thickness_points_subset = thickness_points[::subsample_rate]

    # Step 2: Interpolate ice velocity data at thickness point locations
    # Subset columns of interest first to speed up interpolation operation
    velocity_grid_ss = velocity_grid[["VX", "VY"]]
    # Drop the coordinates we don't need for the interpolation
    velocity_grid_ss = velocity_grid_ss.reset_coords(drop = True)

    # Interpolate ice velocity variables (VX, VY) at all combinations of x and y points from the selected region
    velocity_grid_interpolated = velocity_grid_ss.interp(
        y = (thickness_points_subset["y"]), 
        x = (thickness_points_subset["x"]), 
    ...
    velocity_points_subset = pd.DataFrame({
        # NOTE: for merging we indicate that these are the x coordinates of the ice velocity data
        "x_velocity": x_coords, 
        "y_velocity": y_coords,
        "VX": VX_diag,
        "VY": VY_diag,
    ...
    thickness_velocity_df = pd.concat([
        thickness_points_subset.reset_index(drop = True),
        velocity_points_subset.reset_index(drop = True)],
    ...
    return thickness_velocity_df[["x", "y", "s", "t", "xflux", "yflux", "VX", "VY"]]
...
def df_to_tensor(df, x_min, x_max, y_min, y_max, flux_scale, vel_scale, surface_scale = 2000):
    """
    ...
    # SURFACE: scale surface by fixed scale (1000 m)
    s_tensor = torch.tensor(
        (df.s / surface_scale).to_numpy(),
        dtype = torch.float32
    )

    # FLUXES: scale fluxes by the flux_scale
    # NOTE: higher negative values
    ...
    # VELOCITIES: scale velocities by the flux_scale
    vx_tensor = torch.tensor(
        (df.VX / vel_scale).to_numpy(),
        dtype = torch.float32
    )

    vy_tensor = torch.tensor(
        (df.VY / vel_scale).to_numpy(),
        dtype = torch.float32
    )
    ...
    return torch.cat(
        (x_tensor.unsqueeze(0), 
         ...
         vx_tensor.unsqueeze(0),
         vy_tensor.unsqueeze(0)),
         dim = 0
    )   